...
class PhraseMatcher(object):
    ...
    def _compile(self, fname, max_len=10):
        ...
        with self.bos.transaction(), self.eos.transaction(), \
             self.hashtable.transaction(), self.vocab.transaction():
            n_patterns = 0
            for i, line in enumerate(io.open(fname, 'r', encoding='utf-8')):
                ...
                arr = self.vocab.mget(line.strip().split())
                if None in set(arr):
                    continue
                ...
                if arr_len > max_len:
                    continue
                self.bos[b'{}:{}'.format(arr_len, arr[0])] = b'1'
                self.eos[b'{}:{}'.format(arr_len, arr[-1])] = b'1'
            ...
            for i, tok_int in enumerate(tok_arr):
                ...
                for p_len in self.lengths:
                    ...
                    if j + 1 <= tok_len:
                        ...
                        b_idx = b'{}:{}'.format(p_len, b_int)
                        e_idx = b'{}:{}'.format(p_len, e_int)
